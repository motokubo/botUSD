...
class Logger:
    ...
    def __init__(self, logging_service="luquinhas_trading", enable_notifications=True):
        # Logger setup
        self.Logger = logging.getLogger(f"{logging_service}_logger")
        self.Logger.setLevel(logging.DEBUG)
        self.Logger.propagate = False
        formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
        # default is "logs/luquinhas_trading.log"
        fh = logging.FileHandler(f"luquinhasBot/logs/{logging_service}.log")
